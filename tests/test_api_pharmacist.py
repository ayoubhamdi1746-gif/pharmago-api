...
async def test_pharmacist_verify_success(client: AsyncClient, db_session):
    pid = uuid.uuid4()
    db_session.add(User(id=str(uuid.uuid4()), role="pharmacist", username="ph_verify", identity_id=PH_HASH, hashed_password=hash_password("demo"), is_active=True, pharmacy_id=str(uuid.uuid4()), pharmacist_license_hash=PH_HASH, city="Tunis"))
    db_session.add(LicensedPharmacist(
        pharmacist_license_hash=PH_HASH, full_name_encrypted=b"name", is_active=True,
    ))
    db_session.add(Prescription(id=pid, patient_id="abc", pharmacy_id=PH_HASH, medications=[]))
    db_session.add(PrescriptionVerification(prescription_id=pid, status="PENDING"))
    await db_session.commit()
    resp = await client.post(f"/pharmacist/verify/{pid}", json={}, headers=PH_HDRS)
