...
class PrescriptionVerification(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uid, primary_key=True, default=uuid.uuid4)
    prescription_id: Mapped[uuid.UUID] = mapped_column(Uid, ForeignKey("prescriptions.id"), nullable=False)
    status: Mapped[str] = mapped_column(String(20), default="pending")
    pharmacist_id: Mapped[uuid.UUID | None] = mapped_column(Uid, ForeignKey("users.id"), nullable=True)
...
class PrescriptionEvent(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uid, primary_key=Uuid, default=uuid.uuid4)
