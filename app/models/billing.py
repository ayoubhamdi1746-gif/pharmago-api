...
class PharmacySubscription(Base):
    ...
    pharmacy_id: Mapped[uuid.UUID] = mapped_column(Uuid, nullable=False, index=True, unique=True)
...
class DriverPayout(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(Uuid, primary_key=True, default=uuid.uuid4)
    driver_token_hash: Mapped[str] = mapped_column(String(64), nullable=False, index=True)
    delivery_ticket_id: Mapped[uuid.UUID] = mapped_column(Uuid, ForeignKey("delivery_tickets.id", ondelete="CASCADE"), nullable=False, index=True)
